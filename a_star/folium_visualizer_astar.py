...
class MovementVisualization:
    ...
    def from_coord(self) -> Coordinates:
        return self.step.stop_from.geo

    @property
    def to_coord(self) -> Coordinates:
        return self.step.stop_to.geo

    @property
    def transport_type(self) -> TransportType:
        return self.step.transport_type

    @property
    def transport_info(self) -> str:
        return self.step.route_id
...
class TripVisualization:
    ...
    def __init__(self, steps: List[Step]):  # output.journeys = Iterable[Step]
    ...
    def stops(self) -> List[Stop]:
        ...
        return list(set([step.stop_from for step in self.steps]).union([step.stop_to for step in self.steps]))
    ...
    def _add_stops(self):
        ...
        for step in self.steps:
            visualizers[step.stop_from].dep = step.departure_time
            visualizers[step.stop_to].arr = step.arrive_time
    ...
    def _add_moves(self):
        ...
        visualizers: List[MovementVisualization] = [MovementVisualization(step) for step in self.steps]
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        help=".pcl file of an AstarOutput instance",
    )
    parser.add_argument(
        "-o",
        "--output_dir",
...
def visualize_output(
    ...
    try:
        output: AstarOutput = AstarOutput.read_from_file(filepath=algo_output_path)
    except IOError as ex:
        raise Exception(f"An error occurred while trying to read {algo_output_path}: {ex}")

    visualization = TripVisualization(output.journeys)  # output.journeys = Iterable[Step]
    dep = output.journeys[0].stop_from.name
    arr = output.journeys[-1].stop_to.name

    out_file_path = path.join(visualization_dir, f"{dep}-{arr}.html")
